...
int main(int argc, char *argv[]) {
  int total_correct = 0;
  Dataset* trainingData = load_dataset(argv[1]);
  Dataset* testingData = load_dataset(argv[2]);
  DTNode* root = build_dec_tree(trainingData);
  for (int i = 0; i < testingData->num_items; i++) {
    if (testingData->labels[i] == dec_tree_classify(root, &(testingData->images[i]))) {
      total_correct++;
    }
  }
