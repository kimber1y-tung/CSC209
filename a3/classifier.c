...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_procs; i++) {
        int ret_in = pipe(pipes_in[i]);
        int ret_out = pipe(pipes_out[i]);
    ...
    // TODO
    int start_idx = 0;
    int n_child_data = ceil(testing -> num_items / num_procs);
    for (int i=0; i < num_procs; i++) {
        if (pids[i] == 0) {
            close(pipes_in[i][1]);
            close(pipes_out[i][0]);
            child_handler(training, testing, K, fptr, pipes_in[i][0], pipes_out[i][1]);
            exit(0);
        } else if (pids[i] > 0) {
            close(pipes_in[i][0]);
            write(pipes_in[i][1], &start_idx, sizeof(start_idx));
            start_idx += n_child_data;
            write(pipes_in[i][1], &n_child_data, sizeof(n_child_data));
        }
    }
    ...
    for (int i = 0; i < num_procs; i++) {
        if (pids[i] > 0) {
            ...
            close(pipes_out[i][1]);
            int child_correct;
            read(pipes_out[i][0], &child_correct, sizeof(child_correct));
