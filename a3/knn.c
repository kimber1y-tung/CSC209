...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    int pred_label;
    int child_proc_num;
    read(p_in, &start_idx, sizeof(start_idx));
    read(p_in, &N, sizeof(N));
    if (testing -> num_items < (start_idx + N)) {
        child_proc_num = testing -> num_items;
    } else {
        child_proc_num = start_idx + N;
    }
    for (int i = start_idx; i < child_proc_num; i++) {
        pred_label = knn_predict(training, &testing -> images[i], K, fptr);
        if (pred_label == testing -> labels[i]) {
            correct += 1;
        }
    }
    write(p_out, &correct, sizeof(correct));
    sleep(2);
    return;
}
...
double distance_cosine(Image *a, Image *b){
    ...
    //TODO
    double cos_d = INFINITY;
    double inner_prod = 0.0;
    double a_norm = 0.0;
    double b_norm = 0.0;
    int total = a -> sx * a -> sy;
    for (int i = 0; i < total; i++) {
        inner_prod += a -> data[i] * b -> data[i];
        a_norm += a -> data[i] * a -> data[i];
        b_norm += b -> data[i] * b -> data[i];
    }
    if (a_norm > 0 && b_norm > 0) {
        cos_d = 2 * acos(inner_prod / (sqrt(a_norm) * sqrt(b_norm))) / M_PI;
    }
